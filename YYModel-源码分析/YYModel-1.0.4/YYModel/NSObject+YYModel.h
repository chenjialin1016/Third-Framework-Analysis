 ...
 */
@interface NSObject (YYModel) //已分类的方式实现数据解析
 ...
 */
+ (nullable instancetype)yy_modelWithJSON:(id)json; //json转model
...
/**
 ...
 property, this method will try to convert the value based on these rules:
     ...
     `NSString` -> SEL, Class.
 */
+ (nullable instancetype)yy_modelWithDictionary:(NSDictionary *)dictionary; //字典转model
 ...
 */
- (BOOL)yy_modelSetWithJSON:(id)json; //通过JSON为模型赋值
...
/** 字典设置接受者属性
 ...
 @return Whether succeed.
 */
- (BOOL)yy_modelSetWithDictionary:(NSDictionary *)dic; //通过JSON为字典赋值
 ...
 */
- (nullable id)yy_modelToJSONObject; //模型转NSObject
 ...
 */
- (nullable NSData *)yy_modelToJSONData; //模型转NSData
...
/** 属性转换为JSON字符串
 ...
 inner object to json string.
 */
- (nullable NSString *)yy_modelToJSONString; //模型转NSString
 ...
 */
- (nullable id)yy_modelCopy; //模型深拷贝
 ...
 */
- (void)yy_modelEncodeWithCoder:(NSCoder *)aCoder; //model编码
...
/** 属性解码
 ...
 @return self
 */
- (id)yy_modelInitWithCoder:(NSCoder *)aDecoder; //model解码
...
/** 属性hash值
 ...
 @return Hash code.
 */
- (NSUInteger)yy_modelHash; //model计算hash值
 ...
 */
- (BOOL)yy_modelIsEqual:(id)model; //判断模型是否相等
 ...
 */
+ (nullable NSDictionary<NSString *, id> *)modelCustomPropertyMapper; //属性数据映射，用来定义多样化数据时转换使用
 ...
 */
+ (nullable NSDictionary<NSString *, id> *)modelContainerPropertyGenericClass; //属性自定义类映射，用来实现自定义类的转换声明

/**
 If you need to create instances of different classes during json->object transform,
 use the method to choose custom class based on dictionary data.
 
 如果需要在json->object转换过程中创建不同类的实例，可以使用该方法根据字典数据选择自定义类。
 ...
 */
+ (nullable NSArray<NSString *> *)modelPropertyBlacklist; //属性黑名单，该名单呢的属性不转换为model
...
/**
 ...
 @return An array of property's name.
 */
+ (nullable NSArray<NSString *> *)modelPropertyWhitelist; //属性白名单，只有该名单的属性转换为model

/**
 This method's behavior is similar to `- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic;`, 
 but be called before the model transform. 模型转换之前调用
 ...
 */
- (NSDictionary *)modelCustomWillTransformFromDictionary:(NSDictionary *)dic; //JSON 转为 Model 之前，该方法会被调用
 ...
 */
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic; //JSON 转为 Model 完成后，该方法会被调用。返回false该model会被忽略。同时可以在该model中做一些，转换不能实现的操作，如NSDate类型转换
...
/**
 ...
 @return Returns YES if the model is valid, or NO to ignore this model.
 */
- (BOOL)modelCustomTransformToDictionary:(NSMutableDictionary *)dic; //model 转为 JSON 完成后，该方法会被调用，返回false该model会被忽略。同时可以在该model中做一些，转换不能实现的操作，如NSDate类型转换
