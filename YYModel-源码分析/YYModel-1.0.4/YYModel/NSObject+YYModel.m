...
/// A property info in object model. YYModel属性的元类
@interface _YYModelPropertyMeta : NSObject {
    @package
    NSString *_name;             ///< property's name 属性名称
    YYEncodingType _type;        ///< property's type 属性类型
    YYEncodingNSType _nsType;    ///< property's Foundation type 属性编码类型
    BOOL _isCNumber;             ///< is c number type 是否是C语言类型数值
    Class _cls;                  ///< property's class, or nil 属性的类型
    Class _genericCls;           ///< container's generic class, or nil if threr's no generic class 集合通用类
     ...
     */
    NSString *_mappedToKey;      ///< the key mapped to 映射值
    NSArray *_mappedToKeyPath;   ///< the key path mapped to (nil if the name is not key path) 映射路径
...
/// A class info in object model. YYModel的元类 - 模型对象给的元类信息
@interface _YYModelMeta : NSObject {
    @package
    YYClassInfo *_classInfo; //类信息
    /// Key:mapped key and key path, Value:_YYModelPropertyMeta.
    NSDictionary *_mapper;
    /// Array<_YYModelPropertyMeta>, all property meta of this model. key-属性值
    NSArray *_allPropertyMetas;
    /// Array<_YYModelPropertyMeta>, property meta which is mapped to a key path. 所有属性元类数组
    NSArray *_keyPathPropertyMetas;
    /// Array<_YYModelPropertyMeta>, property meta which is mapped to multi keys. 映射到键值路径的属性元类数组
    NSArray *_multiKeysPropertyMetas;
    /// The number of mapped key (and key path), same to _mapper.count.  多映射到多个键的属性元类数组
    NSUInteger _keyMappedCount; //有效的键值对数量，所谓有效即包含 _getter、_setter、成员变量。 值与 _mapper.count 相同
    /// Model class type.
    YYEncodingNSType _nsType; //数据类型
...
//    1、创建类的基本信息
    ...
    // Get container property's generic class 获取集合属性通用类
...
//    2、遍历类信息的所有属性，并创建所有属性的属性元类meta
    // Create all property metas.
    NSMutableDictionary *allPropertyMetas = [NSMutableDictionary new];
    YYClassInfo *curClassInfo = classInfo;
    //递归遍历类的所有父类，NSObject/NSProxy除外
    ...
    if ([cls respondsToSelector:@selector(modelCustomPropertyMapper)]) {
        ...
        [customMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
           //通过属性名获取属性元类
            ...
            if ([mappedToKey isKindOfClass:[NSString class]]) { //字符串类型的映射关系
            ...
            } else if ([mappedToKey isKindOfClass:[NSArray class]]) { //数组类型的映射关系
 ...
 */
/* 通过runtime的objc_msgSend给meta的setter函数传递相应的value值。即通过objc_msgSend方法发消息进行赋值
 - 1、属性元类型是C语言类型，通过 ModelSetNumberToProperty方法，将NSNumber的值传给model属性元的setter函数
 - 2、属性元的nsType是YYEncodingNSType枚举中的类型，并分别进行处理
 - 3、处理其他类型的情况
...
static void ModelSetValueForProperty(__unsafe_unretained id model, //实例对象
    ...
    if (meta->_isCNumber) { //C语言数值类型
        NSNumber *num = YYNSNumberCreateFromID(value);
        //将NSNumber值传给model属性元的setter函数
        ModelSetNumberToProperty(model, num, meta);
        if (num) [num class]; // hold the number
    } else if (meta->_nsType) { //属性编码类型 是 Foundation系统类型
        if (value == (id)kCFNull) { //值为空的处理
            //objc_msgSend发消息进行赋值为nil
            ((void (*)(id, SEL, id))(void *) objc_msgSend)((id)model, meta->_setter, (id)nil);
        } else { //部位空的处理
            switch (meta->_nsType) { //判断属性的编码类型
                case YYEncodingTypeNSString: //字符串的处理
                ...
                case YYEncodingTypeNSValue: //value、number的处理
                ...
                case YYEncodingTypeNSData: //data数据的处理
                ...
                case YYEncodingTypeNSDate: { //日期类型的处理
                ...
                case YYEncodingTypeNSURL: { //url类型的处理
                ...
                case YYEncodingTypeNSArray: //数组的处理
                ...
                case YYEncodingTypeNSDictionary: //字典的处理
                ...
                case YYEncodingTypeNSSet: //set集合的处理
    ...
    } else { //非Foundation框架提供的类型
        BOOL isNull = (value == (id)kCFNull);
        switch (meta->_type & YYEncodingTypeMask) {
            case YYEncodingTypeObject: { //id类型
                if (isNull) {
                    ((void (*)(id, SEL, id))(void *) objc_msgSend)((id)model, meta->_setter, (id)nil);
            ...
            case YYEncodingTypeStruct: //结构体、联合体、c语言数组
            case YYEncodingTypeUnion:
...
typedef struct {
    void *modelMeta;  ///< _YYModelMeta 元类
    void *model;      ///< id (self) 实力本身
    void *dictionary; ///< NSDictionary (json) 数据字典
} ModelSetContext;

/** 为字典应用函数，将key-value设置为model
...
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ...
    while (propertyMeta) {
        if (propertyMeta->_setter) {
            //核心方法 ModelSetValueForProperty转换
...
//    1、当模型值符合或者接近目标类型时，可做简单的转换，或者直接返回
...
//    3、如果是 自定义类型
    ...
    //遍历和递归其映射表_mapper({属性名：属性元})，通过objc_msgSend发送消息的方式，调用属性的meta->_getter方法进行取值，直至逐一转化成有效的JSON对象。
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
        if (!propertyMeta->_getter) return;
        
        id value = nil;
        //通过objc_msgSend发送消息的方式，调用属性的meta->_getter方法进行取值，
...
/*
 - 1、对json进行非空判断
 - 2、判断JSON类型，对NSString类型的数据进行UTF8编码
 - 3、对NSString和NSData类型的数据使用应该自带的NSJSONSerialization将JSON数据转换为字典
 */
...
//    1、首先对json进行非空判断
    ...
    NSData *jsonData = nil;
//    2、判断json的类型
    if ([json isKindOfClass:[NSDictionary class]]) { //字典
        dic = json;
    } else if ([json isKindOfClass:[NSString class]]) { //字符串
        //对NSString类型的数据进行UTF8编码
        jsonData = [(NSString *)json dataUsingEncoding : NSUTF8StringEncoding];
    } else if ([json isKindOfClass:[NSData class]]) { //data类型
        jsonData = json;
    }
    if (jsonData) {
        //将jsonData转为字典
...
//    2、创建当前类的类对象实例
    ...
    //判断使用者是否根据不同情况自定义了返回类的（子类）类型
    if (modelMeta->_hasCustomClassFromDictionary) {
        //modelCustomClassForDictionary 方法 返回自定义类型
        cls = [cls modelCustomClassForDictionary:dictionary] ?: cls;
    }
//    3、根据cls创建实例
    NSObject *one = [cls new];
//    4、调用yy_modelSetWithDictionary初始化模型 - 为属性赋值
...
//    2、创建_YYModelMeta对象，通过runtime获取当前类 - 创建和获取模型元类（包含类的详细信息）
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:object_getClass(self)];
    //判断当前类的有效属性数量
    if (modelMeta->_keyMappedCount == 0) return NO;
    //判断使用者是否自定义了转换映射 - 是否对数据字典做了额外的处理
    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        //如果有，则返回和使用自定义的字典
        dic = [((id<YYModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]]) return NO;
    }
//  3、创建结构体，ModelSetContext是定义的结构体，包含模型元_YYModelMeta、模型实例id、待处理字典
    //创建模型，设置上下文
...
//    4、如果模型元数值大于等于字典数量关系，CFDictionaryGetCount((CFDictionaryRef)dic)返回字典值的数量
    //比较 元模型的键值数量 & 传入字典的键值数量
    if (modelMeta->_keyMappedCount >= CFDictionaryGetCount((CFDictionaryRef)dic)) {
        /**
         @function CFDictionaryApplyFunction 对字典中的每个键值对调用函数一次。

         @param  theDict 要查的字典。
         @param  applier 要对字典中的每个值调用一次的回调函数。
         @param context 一个指针大小的用户定义值，作为第三个参数传递给applier函数，但此函数不使用它。
         */
        //对字典中的每个值调用ModelSetWithDictionaryFunction方法，将键值对设置为模型
        CFDictionaryApplyFunction((CFDictionaryRef)dic, ModelSetWithDictionaryFunction, &context);
        //如果存在映射keyPath属性元数组
        if (modelMeta->_keyPathPropertyMetas) {
            //对数组中的每个keyPath都执行ModelSetWithPropertyMetaArrayFunction方法
            CFArrayApplyFunction((CFArrayRef)modelMeta->_keyPathPropertyMetas,
                                 CFRangeMake(0, CFArrayGetCount((CFArrayRef)modelMeta->_keyPathPropertyMetas)),
                                 ModelSetWithPropertyMetaArrayFunction,
                                 &context);
        }
        //如果存在key值映射的属性元类数组，对数组中的每个keypath都执行ModelSetWithPropertyMetaArrayFunction方法
        ...
        /**
         @function CFArrayApplyFunction 对数组中的每个元素调用函数一次。
         
         @param theArray 要操作的数组。
         @param range 要将函数应用于的数组中的值范围。
         @param applier 对数组中给定范围内的每个值调用一次的回调函数。如果此参数不是指向正确原型的函数的指针，则行为未定义。如果在应用程序函数期望的范围内存在或不能正确应用的值，则该行为是未定义的。
         @param context 一个指针大小的用户定义值，它作为第二个参数传递给applier函数，但此函数不使用它。如果上下文不是applier函数所期望的内容，则行为是未定义的。
         */
        //每个keyPath都执行ModelSetWithPropertyMetaArrayFunction方法
...
- (id)yy_modelToJSONObject {
     ...
     */
    id jsonObject = ModelToJSONObjectRecursive(self); //返回一个有效的JSON对象
...
+ (NSArray *)yy_modelArrayWithClass:(Class)cls json:(id)json {
    ...
    NSData *jsonData = nil;
//    1、判断json类型
    if ([json isKindOfClass:[NSArray class]]) {//如果是数组类型
        arr = json;
    } else if ([json isKindOfClass:[NSString class]]) {//如果是字符串类型
        //通过UTF8编码
        jsonData = [(NSString *)json dataUsingEncoding : NSUTF8StringEncoding];
    } else if ([json isKindOfClass:[NSData class]]) { //如果是data类型
        jsonData = json;
    }
    if (jsonData) {
        //通过NSJSONSerialization，将二进制数据转为数组
...
+ (NSDictionary *)yy_modelDictionaryWithClass:(Class)cls json:(id)json {
    ...
    NSDictionary *dic = nil;
    NSData *jsonData = nil;
    if ([json isKindOfClass:[NSDictionary class]]) { //字典类型
        dic = json;
    } else if ([json isKindOfClass:[NSString class]]) { //字符串
        //UTF8编码
        jsonData = [(NSString *)json dataUsingEncoding : NSUTF8StringEncoding];
    } else if ([json isKindOfClass:[NSData class]]) { //二进制数据
        jsonData = json;
    }
    if (jsonData) {
        //二进制数据转字典
